...
import { Incident, Solution, IncidentFormData, SearchFilters } from './types/incident';
...
const mockIncidents: Incident[] = [
  {
    ...
    description: 'Large pothole causing vehicle damage near the intersection of Main St and Oak Ave. Multiple cars have been affected.',
    ...
    solutions: [
      {
        ...
        content: 'We\'ve scheduled this for repair next week. Temporary cones will be placed today.',
  ...
  },
  {
    ...
    description: 'Streetlight is out on Elm Street, making the area unsafe at night.',
    category: 'safety',
    priority: 'medium',
    status: 'in-progress',
...
function App() {
  ...
  const handleAddSolution = (incidentId: string, solution: Omit<Solution, 'id' | 'createdAt' | 'helpful' | 'unhelpful'>) => {
  ...
  const handleRateSolution = (incidentId: string, solutionId: string, type: 'helpful' | 'unhelpful') => {
    setIncidents(prev => prev.map(incident => 
      incident.id === incidentId 
        ? {
            ...incident,
  ...
  const filteredIncidents = incidents.filter(incident => {
    const matchesSearch = incident.title.toLowerCase().includes(filters.searchTerm.toLowerCase()) ||
                         ...
                         incident.location.toLowerCase().includes(filters.searchTerm.toLowerCase());
    
    const matchesStatus = !filters.statusFilter || incident.status === filters.statusFilter;
    const matchesCategory = !filters.categoryFilter || incident.category === filters.categoryFilter;
    const matchesPriority = !filters.priorityFilter || incident.priority === filters.priorityFilter;
    
    const matchesDateRange = (!filters.dateFrom || incident.reportedAt >= filters.dateFrom) &&
                            (!filters.dateTo || incident.reportedAt <= filters.dateTo);
    
    const matchesTags = !filters.tags?.length || 
                       filters.tags.some(tag => incident.tags?.includes(tag));
    
    return matchesSearch && matchesStatus && matchesCategory && matchesPriority && matchesDateRange && matchesTags;
  });
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center py-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900 dark:text-white">SnapReport</h1>
                  <p className="text-gray-600 dark:text-gray-400">Report fast, resolve faster</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <NotificationBell />
        ...
        <section className="bg-gradient-to-r from-blue-600 to-blue-800 dark:from-blue-700 dark:to-blue-900 text-white py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-4xl font-bold mb-4">Making Our Community Safer Together</h2>
            <p className="text-xl mb-8 max-w-2xl mx-auto">
              Report incidents instantly, share solutions collaboratively, and build a safer community for everyone.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
              <div className="text-center">
                ...
                <p className="text-blue-100">Quick photo-based incident reporting with smart categorization</p>
              </div>
              <div className="text-center">
                <div className="bg-white bg-opacity-20 p-4 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center">
                ...
                <p className="text-blue-100">Collaborate with neighbors to find and share effective solutions</p>
              </div>
              <div className="text-center">
                <div className="bg-white bg-opacity-20 p-4 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center">
                ...
                <p className="text-blue-100">Live updates and progress tracking with instant notifications</p>
              </div>
            </div>
          </div>
        </section>
